...
function EditComponent(props) {
  ...
  useEffect(() => {
    // Cuando se borra un bloque se lanza lo que esté en el return
    return () => {
      updateTheMeta()
    }
  }, [])
  ...
  function updateTheMeta(){
    // Obtenemos todos los bloques con ese nombre
    const profsForMeta = wp.data.select('core/block-editor')
    .getBlocks().filter(x => x.name == 'ourplugin/featured-professor')
  ...
  //Al ser una función, si se mete en el returnn ocontinua el código.
