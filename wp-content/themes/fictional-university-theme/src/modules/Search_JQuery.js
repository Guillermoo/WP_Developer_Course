...
class Search {
    ...
    getResults() {
        ...
        $.getJSON(universityData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.val(),(results) => {
            ...
            <div class="row">
                <div class="one-third">
                    ...
                    ${results.generalInfo.map(item => `<li><a href="${item.permalink}">${item.title}</a> ${item.type == "post" ? `by <a href="${item.author}">${item.authorName} </a>` : ''}</li>`).join("")}
        ...
        //Este código se usaba cuadno no habíamos creado el search-route.php
        /*$.when(
            ...
            this.resultsDiv.html(`
                ...
                ${combinedResults.map(item => `<li><a href="${item.link}">${item.title.rendered}</a> ${item.type == "post" ? `by <a href="${item.author}">${item.authorName} </a>` : ''}</li>`).join("")}
